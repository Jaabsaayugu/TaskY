...
//   return (
//     <Stack mt={6}>
//       <Container maxWidth="sm" sx={{ minHeight: "100vh", mt: 10 }}>
...
function TaskList(){
  ...
  if (isError) {
    return(
    <Stack component="section" p={7}><Typography variant="h4" textAlign="center" fontWeight="bold"> Something went Wrong! Try Again!</Typography></Stack>
    )
  }
  if (isLoading) {
    return(
  ...
  return(
    <Box component="section" mt={2} >
      <Grid container justifyContent="center" spacing={4} mt={3} px={5} height={200}>
        {
          Array.isArray(data) && data.map((task: Task) => (
            <Task key={task.id} id={task.id} title={task.title} description={task.description} isCompleted={task.isCompleted} isDeleted={task.isDeleted}/>
          ))
        }
